        ...
        foreach ($arResult['QUESTIONS'] as $code => $arQuestion):
                ...
                <div class="row form-group">
                    <div class="col col-12 <?= $oneInLineCol; ?>">
                        <div class="checkbox for-rules<?= $class; ?>">
                            <label>
                                <input class="<?=$class;?> check_box" type="checkbox" checked name="<?= $checkboxCheckCode ?>[]"
                        ...
                        <div class="col col-12 <?= $fieldCol; ?>">
                            <div class="form-select js-select<?= $class; ?>">
                                <select id="<?= $code; ?>" name="<?= $inputName; ?>">
                                    <option selected disabled hidden value=""><?= $arResult['arQuestions'][$code]['COMMENTS']; ?></option>
                                    <?
            ...
            <? elseif ($formFieldInfo['FIELD_TYPE'] == 'file'): ?>
                ...
                <? foreach ($arQuestion['STRUCTURE'] as $arAnswer): ?>
                    <div  <? if ($i > 0): ?>style="display: none;"<? endif; ?> id="row_file_input_<?= $arAnswer['ID'] ?>" class="row form-group flex-vertical row-file-input">
                        <div class="col col-12 <?= $labelCol; ?>">
                            <? if ($i == 0): ?>
                                <label for="<?= $code; ?>"><?= $inputLabel; ?><? if ($arQuestion['REQUIRED'] == "Y"): ?>
                                        <span class="color-orange">*</span><? endif; ?></label>
                        ...
                        <div class="col col-12 <?= $fieldCol; ?>">
                            <div class="form-control--container" <?if($i>0 && $arResult["ID"]==27):?>style="margin-top: -40px;"<?endif;?>>
                                <div class="form-control-file form-control<?= $class ?>">
                                    <i>Выберите файлы</i>
                                    <!--                        --><? //=$arQuestion['HTML_CODE'];?>
                                    <input accept="image/jpeg,image/pjpeg,image/png,image/bmp,image/x-windows-bmp,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/rtf,application/x-rtf,text/richtext,application/excel,application/vnd.ms-excel,application/x-excel,application/x-msexcel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onchange="files_ajax(this.files);" name="form_file_<?= $arAnswer['ID'] ?>"  size="0"
                                           type="<?= $arAnswer['FIELD_TYPE'] ?>" multiple>
                                    <div class="question form-control--question js-toolTip new_question" data-text="Возможно загрузить не более 6 файлов общим размером до 10 МБ. Допустимые форматы файлов: jpg, jpeg, png, bmp, pdf, doc, docx, rtf, xls, xlsx. "></div>
                                    <script>
                    ...
                    <? $i++; ?>
                <? endforeach; ?>
                <div id="output_image" <?if($arParams["WEB_FORM_ID"]==24):?>style="display: flow-root!important;width: 60%!important;position: relative!important;left: 255px!important;"<?endif;?>>
