...
?>
<?php 
//if($dev == 1){ ClassDebug::debug($arParams['ACTION_VARIABLE']); }
?>
<main class="content">
<div class="container">
    ...
    <div class="page-header">
        <div class="pull-right btn-right w-270">
            <a href="<?=$arResult['FOLDER'].$arResult['VARIABLES']['SECTION_CODE'].'/';?>" class="btn btn-label btn-transparent-border block"><span><i class="fa fa-caret-left"></i></span>Вернуться в каталог</a>
        </div>
        <h1 class="overflow"><?=$APPLICATION->ShowTitle();?></h1>
    </div>
    <div class="product-detail product-to-basket">
		<?
...
//                $recommendedData['ALL_LINK'] = '';
        ...
        if (!empty($recommendedData))
        {
            if (!isset($arParams['DETAIL_SHOW_POPULAR']) || $arParams['DETAIL_SHOW_POPULAR'] != 'N')
            {
                ...
                <br><br>
                <div class="section-title"><span>Смотрите также</span></div>
                <?
                $APPLICATION->IncludeComponent(
                    'bitrix:catalog.section',
                    'slider',
