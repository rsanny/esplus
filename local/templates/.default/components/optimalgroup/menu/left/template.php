...
<?if (!empty($arResult)):?>
<div class="hidden-md-up mb-20"><a href="#sidebar-menu" class="btn btn-transparent-border block js-SlideToggle">Меню</a></div>
<ul class="no-list sections-menu" id="sidebar-menu">
<?
$previousLevel = 0;
foreach($arResult as $arItem):
    $link = $arItem["LINK"];
    if ($arParams['MENU_TYPE'] != "shop" && $arParams['IN_SHOP'])
	...
	<? if ($arItem["IS_PARENT"]):?>
        ...
        <?else:?>
			<li class="has-child">
			    ...
			    <a href="<?=$link?>"><?=$arItem["TEXT"]?></a>
			    <? endif;?>
				<ul class="no-list third-level<?if (!$arItem["SELECTED"]):?> none<?endif?>">
		<?endif?>
	<? else:?>
