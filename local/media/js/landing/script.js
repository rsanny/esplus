$(function(){//DOM READEY
    ...
    $('.js-CityIsMy').on('click',function(e){
        ...
        $.ajax({
            ...
            success:function(){
                $('.is-current--city-promo').slideUp();
                $('.header-city a.is-visible').removeClass('is-visible');
            }
        })
    });
...
var onloadCallback = function() {
    if($('#recaptchaResponse').length) {
        review_recaptcha_widget = grecaptcha.render('recaptchaResponse', {
            'sitekey' : G_SITE_KEY
        });
    }
};
...
$(function() {
    ...
    $("body").on("submit", "form[name='SIMPLE_FORM_24']", function (e) {
        if (!$("textarea#g-recaptcha-response").val()) {
            e.preventDefault();
            alert("Проверка на работа не пройдена");
        }
    })
