...
class COptimalGroupMenuComponent extends CBitrixComponent
{
	...
	public function getCacheID($additionalCacheID = false)
	{
		...
		if($this->arParams["MENU_CACHE_TIME"])
		{
            ...
            if(is_array($this->arParams["MENU_CACHE_VARS"]))
			{
				foreach($this->arParams["MENU_CACHE_VARS"] as $code=>$name)
				{
					$strCacheID .= ":".$code."=".$name;
				}
			}
            $strCacheID .= ":MENU_TYPE=".$_SESSION['BXExtra']['SITE']['ID'];
            $strCacheID .= ":MENU_BRANCH=".$_SESSION['BXExtra']['REGION']['IBLOCK']['ID'];
	...
	public function setSelectedItems($bMultiSelect = false)
	{
		...
		$cur_page_no_index = \OptimalGroup\Core::GetCurPage();
		$cur_selected = -1;
		$cur_selected_len = -1;
