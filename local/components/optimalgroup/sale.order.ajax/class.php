 ...
 */
/*
UPDATE/EDITED OPTIMALGROUP
-STORE_LIST
-GRID REMOVED FROM AJAX RESULT
*/
...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function initProperties(Order $order, $isPersonTypeChanged)
	{
		...
		if ($this->isOrderConfirmed)
		{
            ...
            if (!$this->request->get('RULE_FOR_PAY'))
                $this->addError("Для подтверждения заказа необходимо ваше согласие с условиями продажи", self::STRING_BLOCK);
            if (!$this->request->get('RULE_FOR_DATA'))
                $this->addError("Для подтверждения заказа необходимо ваше согласие с обработкой персональных данных", self::STRING_BLOCK);
		}
...
//			$locationChanged = $locValue != $this->request->get('RECENT_DELIVERY_VALUE');
	...
	protected function obtainBasket()
	{
		...
		foreach ($this->order->getBasket() as $basketItem)
		{
            ...
            if (is_numeric($arBasketItem['PROPS']['PICTURE']['VALUE']))
                $arBasketItem['PICTURE'] = CFile::GetPath($arBasketItem['PROPS']['PICTURE']['VALUE']);
            else if ($arBasketItem['PROPS']['PICTURE']['VALUE'])
                $arBasketItem['PICTURE'] = $arBasketItem['PROPS']['PICTURE']['VALUE'];
            if ($arBasketItem['PROPS']['TYPE']['VALUE'] == "PRODUCT" && $arBasketItem['PROPS']['SECTION']['VALUE'] == "ELECT"){
                $arBasketItem['SERVICE'] = $arBasketItem['PROPS']['WITH_SERVICE']['VALUE'];
            }
            $db_old_groups = CIBlockElement::GetElementGroups($arBasketItem['PRODUCT_ID'], true);
            if($ar_group = $db_old_groups->Fetch()){
                $arBasketItem['SECTION'] = $ar_group['NAME'];
            }            
            
            
            $arResult["BASKET_IDS"][$arBasketItem['PRODUCT_ID']] = $arBasketItem['ID'];
	 ...
	 */
    private function MakeStoreAddress($id){
        $arResult = array();
        $arResult['STOCKS'] = \OptimalGroup\Catalog::GetStock($id);
        if ($id != "all"){
            foreach ($id as $currentId){
                foreach ($arResult['STOCKS'][$currentId] as $stock){
                    $arResult['STORE_ID'][$stock['UF_CONTACTS']] = $stock['ID'];
                    $arResult['CONTACTS'][] = $stock['UF_CONTACTS'];
                    $arResult['STORE_LIST'][$stock['ID']] = $stock;
                }
            }
        }
        else {
    ...
    public function StoreList($order){
        $basket = $order->getBasket();
        $basketItems = $basket->getBasketItems();
        $propertyCollection = $order->getPropertyCollection();
        $hasService = false;
        $arIds = $arStore = $arOtherIds = array();
        
        foreach ($basket as $basketItem) {
            $basketPropertyCollection = $basketItem->getPropertyCollection();
            $itemProperties = $basketPropertyCollection->getPropertyValues();
            if ($itemProperties['TYPE']['VALUE'] == "PRODUCT")
                $arIds[] = $basketItem->getProductId();
            else 
                $arOtherIds[] = $basketItem->getProductId();
            
            if ($itemProperties['TYPE']['VALUE'] == "SERVICE")
                $hasService = true;
        }
        if (!$arIds && !$arOtherIds) {
            return false;
        ...
        if (!$arIds && $arOtherIds) {
            $arIds = "all";
            $arStore = $this->MakeStoreAddress($arIds);
        }
        else {
            $arStore = $this->MakeStoreAddress($arIds);
        }
        
        if (empty($arStore['STORE_LIST'])){
            $arStore = $this->MakeStoreAddress("all");
        }
        
        $hasAllProducts = true;
        foreach ($arIds as $ElementId){
            if (!$arStore['STOCKS'][$ElementId]){
                $hasAllProducts = false;
                break;
            }
        }
        
        $arStore['ALL_IN_STOCK'] = true;
        if ($hasService)
            $arStore['HAS_SERVICE'] = true;
        if (!$hasAllProducts){
            $arStore['ALL_IN_STOCK'] = false;
            //unset($arStore['CONTACTS']);
        }      
        
        if ($arStore['CONTACTS']){
            $ContactsList = array();//Need for one address for 2 stores
            $arSelect = Array("ID", "IBLOCK_ID", "NAME", "PROPERTY_*");
            $arFilter = Array("IBLOCK_ID"=>OFFICE_IBLOCK, "ACTIVE"=>"Y", "ID" => $arStore['CONTACTS']);
            $res = CIBlockElement::GetList(array("sort"=>"asc"), $arFilter, false, Array("nPageSize"=>50), $arSelect);
            while($ob = $res->GetNextElement()){ 
                $arFields = $ob->GetFields();
                $arProps = $ob->GetProperties();
                $ContactsList[$arFields['ID']] = array(
                    "ADDRESS" => $arProps['ADDRESS']['VALUE'],
                    "CITY" =>  $arProps['CITY']['VALUE'],
                    "PHONE" =>  $arProps['PHONE']['VALUE'],
                    "COORD" =>  $arProps['COORD']['VALUE'],
                    "BRANCH" =>  $arProps['BRANCH']['VALUE'],
                    "TIME"=> $arProps['TIME_INDIVIDUALS']
                );
            }
            foreach ($arStore['STORE_LIST'] as $k=>$store){
                $arStore['STORE_LIST'][$k]['CONTACTS'] = $ContactsList[$store['UF_CONTACTS']];
            }
        }  
        return $arStore;
    }
	protected function obtainDelivery()
	{
		$arResult =& $this->arResult;
        $arIds = array();
        foreach ($arResult['BASKET_ITEMS'] as $cartItem){
            if ($cartItem['PROPS']['TYPE']['VALUE'] == "PRODUCT") {
                $arIds[] = $cartItem['PRODUCT_ID'];
            }
        }        
        
        $StoreList = $this->StoreList($this->order);
        $arStore = $StoreList['STORE_LIST'];
        $firstStore = reset($arStore);
        $arResult['ALL_IN_STOCK'] = $StoreList['ALL_IN_STOCK'];
        $arResult['HAS_SERVICE'] = $StoreList['HAS_SERVICE'];
		$arResult["STORE_LIST"] = $arStore;
        $arResult['STORE_PRODUCT_ID'] = $arIds;
		/** @var Shipment $shipment */
		$shipment = $this->getCurrentShipment($this->order);
		if (!empty($this->arDeliveryServiceAll) && $arStore)
		{
			foreach ($this->arDeliveryServiceAll as $deliveryObj)
			{
                ...
                if (!empty($buyerStore) && !empty($arStore) && $arStore[$buyerStore])
                {   
        ...
        if (!$buyerStore && !empty($arStore)){
            $sessionPrimary = $_SESSION['PRIMARY_STORE'];
            $lastOrderStoreId = $this->arUserResult['BUYER_STORE']['ID'];
            if ($lastOrderStoreId > 0)
                $buyerStore = $lastOrderStoreId;
            if ($sessionPrimary > 0)
                $buyerStore = $sessionPrimary;
        }
        
        if (!empty($buyerStore) && !empty($arStore) && $arStore[$buyerStore])
        {
            $arResult["BUYER_STORE"] = $arStore[$buyerStore]['ID'];
        }
        else {
            //$arResult["BUYER_STORE"] = $firstStore['ID'];
        }
	...
	protected function obtainPaySystem()
	{
		...
		if (!empty($paySystemList))
		{
			...
			foreach ($paySystemList as $arPaySystem)
			{
				...
				if ($paymentId == $arPaySystem['ID'] && $USER->IsAuthorized())
				{
	...
	protected function initDelivery(Shipment $shipment)
	{
		...
		if (!empty($this->arDeliveryServiceAll))
		{
            ...
            $arStoreList = $this->StoreList($order)['STORE_LIST'];
			
			if (count($arStoreList) > 0)
			{
				if ($this->arUserResult['BUYER_STORE'] <= 0 || !$arStoreList[$this->arUserResult['BUYER_STORE']])
				{
					$this->arUserResult['BUYER_STORE'] = reset($arStoreList);
				}
	...
	protected function getJsDataResult()
	{
		...
		$result['BASKET_ITEMS'] = $arResult['BASKET_ITEMS'];
		...
		$result["ALL_IN_STOCK"] = $arResult['ALL_IN_STOCK'];
        $result["HAS_SERVICE"] = $arResult['HAS_SERVICE'];
        $result["BASKET_IDS"] = $arResult['BASKET_IDS'];
		$result['STORE_LIST'] = $arResult['STORE_LIST'];
        $result['STORE_PRODUCT_ID'] = $arResult['STORE_PRODUCT_ID'];
	...
	protected function makeUserResultArray()
	{
		...
		if ($request->isPost())
		{
			...
			if (empty($arUserResult["PERSON_TYPE_OLD"]) || $arUserResult["PERSON_TYPE_OLD"] == $arUserResult["PERSON_TYPE_ID"])
			{
                ...
                elseif ($this->isOrderConfirmed && empty($paySystemId))
                    $this->addError("Необходимо выбрать способ оплаты", self::STRING_BLOCK);
	...
	protected function showOrderAction()
	{
		...
		if ($order = Order::loadByAccountNumber($orderId))
		{
            ...
            $arOrder["TOTAL_PRICE"] = $order->getPrice();
            $basket = $order->getBasket();
            $basketItems = $basket->getBasketItems();
            $propertyCollection = $order->getPropertyCollection();
            ...
            if ($shipment = $this->getCurrentShipment($order))
			{
                $arOrder["ORDER_STORE_ID"] = $shipment->getStoreId();
            }
			$arResult["ORDER_ID"] = $arOrder["ID"];
			$arResult["ACCOUNT_NUMBER"] = $arOrder["ACCOUNT_NUMBER"];
			$arOrder["IS_ALLOW_PAY"] = $order->isAllowPay()? 'Y' : 'N';
            $arIds = array();
            $arOrder["ORDER_PROPS"] = $arOrderProps;
            foreach ($basket as $basketItem) {
                $arIds[] = $basketItem->getProductId();
            }
		}
        $arOrder['STORE_LIST'] = $this->StoreList($order)['STORE_LIST'];
