...
global $APPLICATION;
    ...
    $uploaddir_new = './uploads_new/'; // . - текущая папка где находится submit.php
    // Создадим папку если её нет

    if (!is_dir($uploaddir)) mkdir($uploaddir, 0777);
    if (!is_dir($uploaddir_new)) mkdir($uploaddir_new, 0777);
...
$sizes=$sum_size/1024/1024+$_POST["sizes"];
    ...
    if($error===false):
        ...
        {
            if (strpos($arImg["tmp_name"], "doc") !== false || strpos($arImg["tmp_name"], "docx") || strpos($arImg["tmp_name"], "rtf")) {
                $files_make_array_new[$key]=$arImg;
                $files_make_array_new[$key]["tmp_name"] = "/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--doc.png";
                $files_file[$key]=$arImg;
            }
            elseif(strpos($arImg["tmp_name"], "xls") !== false || strpos($arImg["tmp_name"], "xlsx"))
            {
                $files_make_array_new[$key]=$arImg;
                $files_make_array_new[$key]["tmp_name"] = "/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--exl.png";
                $files_file[$key]=$arImg;
            }
            elseif(strpos($arImg["tmp_name"], "pdf") !== false)
            {
                $files_make_array_new[$key]=$arImg;
                $files_make_array_new[$key]["tmp_name"] = "/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--pdf.png";
                $files_file[$key]=$arImg;
            }
            else
            {
        ...
        {
            $id_save=CFile::SaveFile($img_arr, "form");
            if(in_array("/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--doc.png",$img_arr)
            || in_array("/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--exl.png",$img_arr)
            || in_array("/var/www/esplus/data/www/esplus.extyl.pro/local/media/icons/icon-file--pdf.png",$img_arr)
            )
            {
                $fid_file[]=$id_save;
                $fid[] = $id_save;
        ...
        foreach($files_file as $file_arr)
        {
            $id_save_file[]=CFile::SaveFile($file_arr, "form");
        }

        //$APPLICATION->set_cookie("ID_LOAD_FILES", $fid, time()+60*60*24*30*12*2, "/");

        if(!isset($_COOKIE["imageIdLoad"])) {
            $fd_cookie=implode(",", $fid);
            $fid_file_string=implode(",",$fid_file);
            $id_save_file_string=implode(",",$id_save_file);
            $fd_cookie=str_replace( $fid_file_string, $id_save_file_string, $fd_cookie);
            setcookie("imageIdLoad", $fd_cookie, time() + 3600,"/");
        }
        else
        {
            $arrLoadImg=explode(",",$_COOKIE["imageIdLoad"]);
            $newFileIDs=array_merge($arrLoadImg,$fid);
            $fd_cookie=implode(",", $newFileIDs);
            $fid_file_string=implode(",",$fid_file);
            $id_save_file_string=implode(",",$id_save_file);
            $fd_cookie=str_replace( $fid_file_string, $id_save_file_string, $fd_cookie);
            setcookie("imageIdLoad", $fd_cookie, time() + 3600,"/");
